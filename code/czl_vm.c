...
static char czl_fun_local_vars_backup(czl_gp *gp, czl_fun *fun)
{
    ...
    if (fun->type != CZL_SYS_FUN)
    {
        ...
        if (fun->foreach_cnt)
        {
            ...
            {
                tmp->type = f->type;
                tmp->val.ext.v1.ptr = f->c;
                tmp->val.ext.v2.u32 = f->para.inx;
            }
        }
    }
...
static void czl_fun_local_vars_restore(czl_gp *gp, czl_fun *fun)
{
    ...
    if (fun->type != CZL_SYS_FUN)
    {
        ...
        if (fun->foreach_cnt)
        {
            ...
            {
                f->type = vars->type;
                f->c = (czl_exp_ele*)vars->val.ext.v1.ptr;
                f->para.inx = vars->val.ext.v2.u32;
            }
        }
    }
...
static void czl_coroutine_paras_copy(czl_fun *fun, czl_var *vars)
{
    ...
    for (i = 0; i < j; ++i)
    {
        ...
        fun->foreachs[i].c = (czl_exp_ele*)vars[i].val.ext.v1.ptr;
        fun->foreachs[i].para.inx = vars[i].val.ext.v2.u32;
    }
}
...
static void czl_coroutine_paras_backup(czl_fun *fun, czl_var *vars)
{
    ...
    for (i = 0; i < j; ++i)
    {
        ...
        vars[i].val.ext.v1.ptr = fun->foreachs[i].c;
        vars[i].val.ext.v2.u32 = fun->foreachs[i].para.inx;
    }
}
...
char czl_exp_unary_const_merge(czl_gp *gp, czl_exp_node *node)
{
    ...
    for (p = node; p; p = p->left, cnt++)
    {
        if (p == node)
        {
            if (!czl_opt_cac_funs[p->type](gp, &res, var))
                return 0;
        }
        else
        {
            if (!czl_opt_cac_funs[p->type](gp, &res, &res))
                return 0;
        }
    }
...
)
{
    ...
    else if (loop->k)
    {
        ...
        if (!czl_ass_cac(gp, lo, &tmp))
            return -1;
        if (CZL_REG_VAR == loop->k->kind && loop->k->res->type != CZL_OBJ_REF)
            lo = loop->k->res;
        else if (!(lo=czl_get_opr(gp, loop->k->kind, loop->k->res)))
        {
            czl_runtime_error_report(gp, loop->k);
            return -1;
        }
...
)
{
    ...
    case 1: case 2:
        ...
        break;
    default: //3
        ret = czl_bytes_read(gp, f->fp, lo, 1);
        break;
    }
...
)
{
    ...
    if (2 == loop->flag)
    {
        ...
        if (loop->k)
        {
            if (CZL_INS_VAR == loop->k->kind)
                lo = ((czl_ins_var*)loop->k->res)->var;
            else
                lo = loop->k->res;
        }
        else
    ...
    case CZL_STRING:
        ...
        break;
    case CZL_FILE:
        ret = czl_foreach_file(gp, lo, CZL_FIL(ro->val.Obj));
        break;
    case CZL_ARRAY_LIST:
...
)
{
    ...
    while (i < cnt)
    {
        ...
        CZL_RT_FREE(gp, f->k, sizeof(czl_exp_ele));
